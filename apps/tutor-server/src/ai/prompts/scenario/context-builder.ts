...
export function buildScenarioContext(
  ...
  const focusWords = currentBucket.words.filter(w => !usedSet.has(w.toLowerCase()))
  const currentBucketUsedCount = currentBucket.words.filter(w =>
    usedSet.has(w.toLowerCase())
  ).length
  const currentBucketProgress =
    currentBucket.words.length > 0 ? currentBucketUsedCount / currentBucket.words.length : 0

  let nextBucketWords: string[] = []
  if (currentBucketProgress >= 0.5 && currentActIndex < buckets.length - 1) {
    nextBucketWords = buckets[currentActIndex + 1].words.filter(w => !usedSet.has(w.toLowerCase()))
  }
