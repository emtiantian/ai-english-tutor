...
export default defineConfig({
  ...
  theme: {
    ...
    animation: {
      keyframes: {
        ...
        blink: '{50%{opacity:0}}',
        'fade-in':
          '{from{opacity:0;transform:translate(-50%,-50%) scale(0.9)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}'
      },
      durations: {
  ...
  shortcuts: {
    ...
    // Pill button
    'btn-pill':
      'rounded-30px border border-white/40 bg-white/8 backdrop-blur-10px text-white cursor-pointer transition-all',
    'btn-pill-active': 'scale-96 bg-white/15',
    // Blue accent button
    'btn-primary':
      'h-40px px-20px rounded-20px bg-primary-80 text-white text-14px cursor-pointer transition-all flex-shrink-0 hover:bg-primary active:scale-95',
    // Input field
    'input-pill':
      'flex-1 h-40px px-14px rounded-20px bg-white/8 text-white text-15px outline-none placeholder-white/40',
    // Dot indicator
