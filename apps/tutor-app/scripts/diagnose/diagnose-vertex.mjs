      ...
      if (drawCount <= 3) {
        ...
        for (let i = 0; i < numUniforms; i++) {
          ...
          if (info.type === gl.FLOAT_MAT4) {
            try { value = Array.from(gl.getUniform(prog, loc)); } catch(e) {}
          } else if (info.type === gl.SAMPLER_2D) {
            try { value = gl.getUniform(prog, loc); } catch(e) {}
          }
          uniformInfo.push({name: info.name, type: info.type, value: value ? value.slice(0, 4) : null});
        }
