...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public drawMeshWebGL(model: Readonly<CubismModel>, index: number): void {
    ...
    if (!this.isGeneratingMask()) {
      const texNo = model.getDrawableTextureIndex(index);
      const tex = this.getBindedTextures().get(texNo);
      if (!tex && index === 0) {
        console.warn('[Live2D] No texture bound for drawable', index, 'textureNo=', texNo, 'textures=', this.getBindedTextures().size);
      }
    }
