...
AI English Tutor 是个人使用的英语场景对话练习应用。第一版核心闭环是“选择场景 → LLM 开场 → TTS 播放 → 用户文字或语音输入 → ASR 转写 → LLM 继续 → 查看基础单词”。不要把主线改成技能短课或课程闯关，除非用户明确改变方向。
...
- 范围取舍以 `doc/优化计划.md` 为准。多 Provider、缓存、持久化、经验等级、暂停续玩等能力暂不接入主要流程。
- 第一版生产组合固定为 DeepSeek、小米 TTS 和浏览器 ASR；Voice Design 只来自用户音色选择，不与场景或 LLM 人格绑定。
